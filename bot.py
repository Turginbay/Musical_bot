...
async def start_command(message: types.Message):
    ...
    await message.answer("Қалайсың Балапан? 🐣", reply_markup=menu)


# 📁 Файл жіберу (inline батырма арқылы)
@dp.callback_query(lambda c: c.data == "file")
async def send_file(callback: types.CallbackQuery):
    file_path = r"C:\Users\bekza\Telegramm_bot\Musical_Bot.docx" # Файл атауы
...
async def send_file(callback: types.CallbackQuery):
    file_path = r"C:\Users\bekza\Telegramm_bot\Musical_bot.pptx"  # Файл атауы
    await bot.send_document(callback.message.chat.id, document=types.FSInputFile(file_path))
    await callback.answer()
...
async def ask_for_song(message: types.Message):
    ...
    await message.answer("Музыканы жаз..", reply_markup=ReplyKeyboardRemove())  # Боттың жаңа хабарламасы
...
async def asc_for_Me (message: types.Message):
     await message.answer( "Балапан егер жалоб болса немесе өтініш болса жазағой..😉\n Боқтап жазу жоғынан, Барлығыңды жақсы көремін \n @FTbekzat\n https://t.me/+-bb5w6x5MCIwNzky",  reply_markup=menu_kb)

# Қолданушы әннің атын жазғанда ғана іздеу
@dp.message()
async def search_music(message: types.Message):
    if message.text in ["Қандай Әнді Іздегің келеді Балапан 🥰", "Ойын ойнайсың ба? Балапан 🙃", "⚙️ Баптаулар", "Менімен Байланыс жасайсын ба? Балапан ☺️","Тағы Музыка Керекпе?😁", "Әлде ойын?🤪", "Әлде Көңіл күйіңе байланысты музыка таңдайсың ба?😅", "Көңіл күй арқылы керекпа әлде?", "Ән іздеу ☺️", "Көңіл күй😏", "📢 Достарыңды шақыр","ADMIN 💘", "Көңіл күй 🥺", "Файл 📋", "file","files"]:
        return  
